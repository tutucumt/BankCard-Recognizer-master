...
class APP(UIMainWindow):
    ...
    def predict(self):
        ...
        if self.diaplay_img.item.activate:
            ...
            #thread = AutoLocateThread(self.img_name, self.model_path, self., self.result_line)
            thread.run()
    ...
    def predict_selected_img(self, pred_img):
        ...
        #thread = PredictThread(self.img_array, IMG_SIZE, self.model_path["crnn"], self.result_line)
        thread.run()
